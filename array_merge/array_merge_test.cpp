...
// Test to handle one longer list
...
// Test to handle multiple copies of longer list
...
// Test to handle different list orders
...
// Test to handle lists with different sizes
TEST(ArrayMerge, Handle_different_sizes) {
  ...
  // Free allocated memory for individual arrays
  for (int i = 0; i < num_arrays; ++i) {
    free(a[i]);
  }
}

// Test to handle lists with different sizes in reverse
